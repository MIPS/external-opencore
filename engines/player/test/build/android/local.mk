...
# Run GENERATE_PARALLEL_TESTS_CMD
OUTPUT = $(shell "$(GENERATE_PARALLEL_TESTS_CMD)")

# Obtain the PARALLEL_TEST_IDS from GEN_TEST_FILE.
PARALLEL_TEST_IDS = $(shell cat $(GEN_TEST_FILE) | cut -d' ' -f1)
...
$(PARALLEL_TEST_IDS): TEST_NUMBER = $(shell grep "^$@ " $(GEN_TEST_FILE) | cut -d' ' -s -f1)
