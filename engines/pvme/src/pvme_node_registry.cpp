...
PVMENodeRegistry::PVMENodeRegistry()
{
    iType.reserve(20);
    iLogger = PVLogger::GetLoggerObject("pvmenoderegistry");
}
...
void PVMENodeRegistry::AddLoadableModules(const OSCL_String& aConfigFilePath)
{
    ...
    for (unsigned int i = 0; i < libList.Size(); i++)
    {
        ...
        if (lib->LoadLib(libList.GetLibraryPathAt(i)) == OsclLibSuccess)
        {
            ...
            {
                struct PVMetadataEngineNodeSharedLibInfo *libInfo = (struct PVMetadataEngineNodeSharedLibInfo *)oscl_malloc(sizeof(struct PVMetadataEngineNodeSharedLibInfo));
                if (NULL != libInfo)
                {
                    libInfo->iLib = lib;
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMENodeRegistry::AddLoadableModules() QueryInterface() of PV_NODE_POPULATOR_INTERFACE for library %s failed.", libList.GetLibraryPathAt(i).get_cstr()));

            }
        }
        else
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_MLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMENodeRegistry::AddLoadableModules() LoadLib() of library %s failed.", libList.GetLibraryPathAt(i).get_cstr()));
        }
        lib->Close();
        OSCL_DELETE(lib);
    }
...
#endif
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::AddLoadableModules() OUT"));
}
...
void PVMENodeRegistry::RemoveLoadableModules()
{
    ...
    {
        struct PVMetadataEngineNodeSharedLibInfo *libInfo = iNodeLibInfoList.front();
...
#endif
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::RemoveLoadableModules() OUT"));
}


PVMFStatus PVMENodeRegistry::QueryRegistry(PVMFFormatType& aInputType, PVMFFormatType& aOutputType, Oscl_Vector<PVUuid, OsclMemAllocator>& aUuids)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::QueryRegistry() IN"));
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::QueryRegistry() OUT"));
    if (matchfound)
    {
        return PVMFSuccess;
    }
...
PVMFNodeInterface* PVMENodeRegistry::CreateNode(PVUuid& aUuid)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::CreateNode() IN"));
    ...
    if (iFoundFlag)
    {
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::CreateNode() OUT"));
...
bool PVMENodeRegistry::ReleaseNode(PVUuid& aUuid, PVMFNodeInterface* aNode)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::ReleaseNode() IN"));
    bool iFoundFlag = false;
    uint32 NodeSearchCount = 0;
    ...
    if (iFoundFlag)
    {
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMENodeRegistry::ReleaseNode() OUT"));
        return del_stat;
    }
...
PVMERecognizerRegistry::PVMERecognizerRegistry()
        : OsclTimerObject(OsclActiveObject::EPriorityNominal, "PVMERecognizerRegistry")
{
    AddToScheduler();
    ...
    iLogger = PVLogger::GetLoggerObject("pvmerecognizerregistry");
}
...
void PVMERecognizerRegistry::AddLoadableModules(const OSCL_String& aConfigFilePath)
{
    ...
    for (unsigned int i = 0; i < libList.Size(); i++)
    {
        ...
        if (lib->LoadLib(libList.GetLibraryPathAt(i)) == OsclLibSuccess)
        {
            ...
            if (result == OsclLibSuccess && interfacePtr != NULL)
            {
                struct PVMetadataEngineRecognizerSharedLibInfo *libInfo = (struct PVMetadataEngineRecognizerSharedLibInfo *)oscl_malloc(sizeof(struct PVMetadataEngineRecognizerSharedLibInfo));
                if (NULL != libInfo)
                {
                    libInfo->iLib = lib;
    ...
    OSCL_UNUSED_ARG(aConfigFilePath);
#endif
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::AddLoadableModules() OUT"));
}
...
void PVMERecognizerRegistry::RemoveLoadableModules()
{
    ...
    {
        struct PVMetadataEngineRecognizerSharedLibInfo *libInfo = iRecognizerLibInfoList.front();
...
#endif
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::RemoveLoadableModules() OUT"));
}

PVMFStatus PVMERecognizerRegistry::QueryFormatType(OSCL_wString& aSourceURL, PVMERecognizerRegistryObserver& aObserver, OsclAny* aCmdContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::QueryFormatType() IN"));
    if (iObserver != NULL)
    {
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::QueryFormatType() OUT"));
    return PVMFSuccess;
}

PVMFStatus PVMERecognizerRegistry::QueryFormatType(PVMFCPMPluginAccessInterfaceFactory* aDataStreamFactory, PVMERecognizerRegistryObserver& aObserver, OsclAny* aCmdContext)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::QueryFormatType() IN"));
    if (iObserver != NULL)
    {
    ...
    // delete the previous DataStreamFactory created by PVMERecognizerRegistry
    if (iFileDataStreamFactory)
    {
        OSCL_DELETE(OSCL_STATIC_CAST(PVMIDataStreamSyncInterfaceRefFactory*, iFileDataStreamFactory));
        iFileDataStreamFactory = NULL;
...
void PVMERecognizerRegistry::CancelQuery(OsclAny* aContext)
{
    if (iObserver == NULL)
    {
...
void PVMERecognizerRegistry::Run()
{
...
void PVMERecognizerRegistry::RecognizerCommandCompleted(const PVMFCmdResp& aResponse)
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::RecognizerCommandCompleted() IN"));
    iSourceFormatType = PVMF_MIME_FORMAT_UNKNOWN;
    ...
    if (aResponse.GetCmdId() == iRecognizeCmdId)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::RecognizeCommandCompleted() - Recognize returned Success"));
        ...
        else if (aResponse.GetCmdStatus() == PVMFErrCancelled)
        {
            ...
            // calling Run
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::RecognizerCommandCompleted() - Recognize returned Cancelled - OUT"));
            return;
        }
    }

    RunIfNotReady();
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG, (0, "PVMERecognizerRegistry::RecognizerCommandCompleted() OUT"));
}
