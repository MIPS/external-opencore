...
void test_base::InitializeLogs()
{
    ...
    if (logfile)
    {
        ...
        OsclRefCounter *appenderRefCounter = OSCL_NEW(TextCounterType, (lLoggerAppender));
        refCounter = appenderRefCounter;
    }
    else
    {
        //Console Log
        lLoggerAppender = OSCL_NEW(StdErrAppender<TimeAndIdLayout, 1024>, ());
        typedef OsclRefCounterSA<AppenderDestructDealloc<StdErrAppender<TimeAndIdLayout, 1024> > > ErrCounterType;
        OsclRefCounter *appenderRefCounter = OSCL_NEW(ErrCounterType, (lLoggerAppender));
        refCounter = appenderRefCounter;
    }
