...
OSCL_EXPORT_REF int16 ParseAVCFSI(uint8* buffer, uint32 length, AVCConfigInfo* iAVCConfigInfo)
{
    ...
    int32 nal_length = 0, bytesLeft = 0;
    uint32 i;
    ...
    do
    {
        if (nal[0] == 0 && nal[1] == 0)
        {
            ...
            if (nal[i-1] == 1)
            {
                ...
                nal_length = 0;
                // search for the next start code 00000001
                while (nal_length < (int32)(length - i))
                {
                    if ((nal[nal_length] == 0) && (nal[nal_length+1] == 0))
                    {
                        if (nal[nal_length+2] == 1)
                            break; //start code 0x000001
                        else if ((nal[nal_length+2] == 0) && (nal[nal_length+3] == 1))
                            break; //start code 0x00000001
                    }
                    nal_length++;
                }

                //here we have all nals together as next nal
                bytesLeft = length - i - nal_length;
            }
            else
            {
                OSCL_FREE(temp);
        ...
        Parser_EBSPtoRBSP(nal, (int32*) &nal_length);

        psBits.data = nal;
        psBits.numBytes = nal_length;
        psBits.bitBuf = 0;
        psBits.bitPos = 32;
        psBits.bytePos = 0;
        psBits.dataBitPos = 0;

        if (!getNALType(nal, nal_length, &nal_type))
        {
            OSCL_FREE(temp);
            return MP4_INVALID_VOL_PARAM;
        }
