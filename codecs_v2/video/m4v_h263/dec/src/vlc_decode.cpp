...
PV_STATUS VlcDecTCOEF_FLV1(BitstreamDecVideo *stream, Tcoef *pTcoef/*, int intra*/)
{
    uint code;
    int levelSize;
    const VLCtab2 *tab;
    ...
    /* escape mode 4 - H.263 type */
    levelSize = pTcoef->sign;
    code = BitstreamReadBits16_INLINE(stream, 7); /* Run + last*/
    pTcoef->last = code >> 6;
    pTcoef->run = code & 0x3F;

    pTcoef->sign = 0;
    if (levelSize == 0)
    {
        pTcoef->level = (int) BitstreamReadBits16_INLINE(stream, 7); /* Level */
        if (pTcoef->level > 63)
        {
            pTcoef->sign = 1;
            pTcoef->level = 128 - pTcoef->level;
        }
    }
    else
    {
        pTcoef->level = (int) BitstreamReadBits16_INLINE(stream, 11); /* Level */
        if (pTcoef->level > 1023)
        {
            pTcoef->sign = 1;
            pTcoef->level = 2048 - pTcoef->level;
        }
    }
    return PV_SUCCESS;
