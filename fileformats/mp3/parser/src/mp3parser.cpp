...
MP3ErrorType MP3Parser::ParseMP3File(PVFile * fpUsed, bool aEnableCRC)
{
    ...
    if ((oscl_memcmp((pBuf + VBRI_HEADER_OFFSET), STR_VBRI_HEADER_IDENTIFIER, VBR_HEADER_SIZE) == 0))
    {
        ...
        else
        {
            ...
            // this frame is a Vbri header, the MP3 media data should start after this frame
    ...
    iCurrFrameNumber = 0;
    //reset minimum bytes required for init
    iMinBytesRequiredForInit = KMAX_MP3FRAME_LENGTH_IN_BYTES;
    return MP3_SUCCESS;
}
 ...
 * SIDE EFFECTS:
 ***********************************************************************/
int32 MP3Parser::GetNextBundledAccessUnits(uint32 *n, GAU *pgau, MP3ErrorType &error, int32& aEOCFrameIndex, uint32 &aNumPaddingFrames)
{
    uint32 nBytesRead = 0;
    uint32 framets = 0;
    ...
    for (; (framesRead < numFrames) && (iLength > 0); framesRead++)
    {
        ...
        else
        {
            ...
            if (framesRead >= iPaddingFrames)
            {
                ...
                aNumPaddingFrames = iPaddingFrames;
            }
            break;
        }
...
BEGIN:
    ...
    // In Shoutcast, the file position/file size will wrap around at 4GB
    // file size should wrap before file position
    if ((0 != fileSz) && ((fileSz - currentFilePosn) < (mp3FrameSizeInBytes - MP3_FRAME_HEADER_SIZE)))
    {
        // At EOF
        iCurrFrameNumber--;
        framesize = 0;
...
MP3ErrorType MP3Parser::mp3FindSync(uint32 seekPoint, uint32 &syncOffset, PVFile* aFile)
{
    ...
    {
        if (maxSearchOffset < iInitSearchFileSize && j + iMaxSyncBufferSize > maxSearchOffset)
        {
            // update the data to be searched next time.
            iInitSearchFileSize -= maxSearchOffset;
            if (pSyncBuffer)
            {
                OSCL_ARRAY_DELETE(pSyncBuffer);
                pSyncBuffer = NULL;
            ...
            for (i = 0; i < (BufferSize - 3); i++)
            {
                if (pSyncBuffer[i] == 0xFF)
                {
                    ...
                    if ((pSyncBuffer[i+1] & 0xF0) == 0xF0)
                    {
                        ...
                        {
                            // read pointer is at begining of valid audio frame
                            break;
                        }
                        else if (mp3Err == MP3_INSUFFICIENT_DATA)
                        {
